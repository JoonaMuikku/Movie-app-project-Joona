...
export const getPublicFavorites = async (req, res, next) => {
    ...
    try {
        const userQuery = `SELECT user_id, is_public FROM users WHERE username = $1;`;
        ...
        const { user_id, is_public } = userResult.rows[0];
        ...
        res.status(200).json({ favorites: favoritesResult.rows });
    } catch (error) {
        next(error);
    }
};
